...
use crate::names::{DatabaseId, FullObjectName, PartialObjectName, QualifiedObjectName, SchemaId};
...
/// To simplify the design, all SQL objects are under "fp"
/// database.
const FLOPPY_DB_NAME: &str = "fp";
const FLOPPY_DB_ID: DatabaseId = DatabaseId(100);

/// To simplify the design, all user tables are under
/// "public" schema.
const FLOPPY_SCHEMA_NAME: &str = "public";
const FLOPPY_SCHEMA_ID: SchemaId = SchemaId(101);

/// A catalog keeps track of SQL objects available to the
/// planner.
///
/// The SQL standard mandates a catalog hierarchy of exactly
/// three layers. A catalog contains databases, database
/// contain schemas, and schemas contain catalog items, like
/// table, indexes.
///
...
///
/// To simplify the design of Floppy, all SQL objects are
/// put into the database of "test", and all use created
/// tables are in the "public" schema (the "pg_catalog"
/// schema contains the system tables and all the built-in
...
///
/// "item" has a very specific meaning in the context of a
/// SQL catalog, and refers to the various entities that
/// belong to a schema.
pub trait CatalogItem {
    /// Returns the fully qualified name of the catalog
    /// item.
    ...
    /// Returns a description of the result set produced by
    /// the catalog item.
    ///
    /// If the catalog item is not of a type that produces
    /// data (i.e., an index), it returns an error.
    ...
    /// A normalized SQL statement that describes how to
    /// creat the catalog item.
